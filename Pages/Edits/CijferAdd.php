...
class NieuwEngelsCijfer
{
    ...
    public  function  NewGradeStudent($mysql, $sutdentID){


        $stmt = $mysql->prepare("INSERT INTO `engels`(`ID`, `Cijfer`, `Datum`, `toetsNaam`, `Studenten_ID`, `Leraren_ID`) VALUES (DEFAULT,?,?,?, DEFAULT ,?);");

        if (false === $stmt) {
            throw new Exception($mysql->error);
        }

        $bind = $stmt->bind_param('sss', $Cijfer, $Datum, $Toetsnaam, $IdLeraar);
        if (false === $bind) {
            throw new Exception($stmt->error);
        }
